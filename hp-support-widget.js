(function () {
  ...
  function init() {
    ...
    var css = ''
      ...
      /* Header */
      + '.hp-panel-header{background:#1a1a2e;padding:24px 20px;color:#fff!important;position:relative;flex-shrink:0;}'
      + '.hp-avatar{width:44px;height:44px;background:#202b31;border-radius:12px;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:15px;margin-bottom:12px;color:#fff!important;letter-spacing:.5px;}'
      + '.hp-panel-header h2{margin:0;font-size:22px;font-weight:700;line-height:1.35;color:#fff!important;}'
      + '.hp-close-x{position:absolute;top:14px;right:14px;width:30px;height:30px;background:rgba(255,255,255,.12);border:none;color:#fff!important;border-radius:50%;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;transition:background .15s;font-family:inherit;}'
      + '.hp-close-x:hover{background:rgba(255,255,255,.22);}'

      /* Views */
      + '.hp-view{display:none;flex-direction:column;flex:1;overflow-y:auto;padding:14px;gap:10px;scrollbar-width:thin;}'
      + '.hp-view.hp-view-active{display:flex;}'

      /* Home view cards */
      + '.hp-section-label{font-size:11px;font-weight:600;color:#999;text-transform:uppercase;letter-spacing:.8px;padding:4px 2px 0;}'
      + '.hp-card{background:white;border-radius:14px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.06);}'
      + '.hp-card a{display:flex;align-items:center;padding:15px 14px;text-decoration:none;border-bottom:1px solid #f2f2f2;transition:background .15s;gap:12px;}'
      ...
      + '.hp-submit-card{background:white;border-radius:14px;padding:15px;box-shadow:0 1px 4px rgba(0,0,0,.06);display:flex;align-items:center;gap:13px;text-decoration:none;transition:background .15s;}'
      + '.hp-submit-card:hover{background:#fafafa;}'
      + '.hp-submit-icon{width:44px;height:44px;background:#eff2ff;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;}'
      + '.hp-submit-text strong{display:block;font-size:14px;font-weight:600;color:#111;}'
      + '.hp-submit-text span{font-size:12px;color:#4a6cf7;display:flex;align-items:center;gap:5px;margin-top:3px;}'
      + '.hp-online-dot{display:inline-block;width:7px;height:7px;background:#22c55e;border-radius:50%;flex-shrink:0;}'

      /* Tickets view */
      + '.hp-state-wrap{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;padding:20px;text-align:center;}'
      + '.hp-state-msg{font-size:13px;color:#888;line-height:1.5;}'
      + '.hp-state-msg a{color:#4a6cf7;text-decoration:none;}'
      + '.hp-spinner{width:28px;height:28px;border:3px solid #eee;border-top-color:#4a6cf7;border-radius:50%;animation:hp-spin .7s linear infinite;}'
      + '@keyframes hp-spin{to{transform:rotate(360deg);}}'
      + '.hp-ticket-list{display:flex;flex-direction:column;gap:8px;}'
      + '.hp-ticket-item{background:white;border-radius:12px;padding:13px 14px;display:flex;align-items:center;gap:12px;box-shadow:0 1px 4px rgba(0,0,0,.06);}'
      + '.hp-ticket-item-body{flex:1;min-width:0;}'
      + '.hp-ticket-item-title{font-size:13px;font-weight:600;color:#111;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}'
      + '.hp-ticket-item-date{font-size:11px;color:#aaa;margin-top:2px;}'
      + '.hp-ticket-item-link{font-size:12px;color:#4a6cf7;text-decoration:none;flex-shrink:0;font-weight:500;white-space:nowrap;}'
      + '.hp-ticket-item-link:hover{text-decoration:underline;}'
      + '.hp-tickets-header{font-size:11px;font-weight:600;color:#999;text-transform:uppercase;letter-spacing:.8px;padding:2px 2px 0;flex-shrink:0;}'

      /* Footer nav */
      + '.hp-footer-nav{display:flex;background:white;border-top:1px solid #ebebeb;padding:8px 0 10px;flex-shrink:0;}'
      + '.hp-footer-nav a,.hp-nav-btn{flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;text-decoration:none;color:#bbb;font-size:11px;font-weight:500;padding:4px 0;transition:color .15s;background:none;border:none;cursor:pointer;font-family:inherit;}'
      + '.hp-footer-nav a.hp-active,.hp-nav-btn.hp-active,.hp-footer-nav a:hover,.hp-nav-btn:hover{color:#1a1a2e;}'
      + '.hp-footer-nav svg,.hp-nav-btn svg{width:20px;height:20px;}';
